...
| **`weight`** | `list` / `ndarray` | 推奨 | 知識ノード間の重み行列（$N \times N$） | 知識間のエッジ線（動的太さ・カラー強調 ＆ 数値ラベルバッジ表示） |
...
| **`threshold`** | `float` | 推奨 | 知識活性化の判定閾値（デフォルト `0.18`、実験設定ファイル `config_used_*.yaml` がある場合はその `SAP.THRESHOLD` が最優先適用） | バーチャート・折れ線グラフ上の**赤色基準線** |
