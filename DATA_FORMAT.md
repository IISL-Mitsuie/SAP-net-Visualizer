...
- **推奨構成のメリット**:
  - ビューアー起動時にフォルダを選択するだけで、フォルダ直下の最新ログとパラメータを自動検出して一括ロード（※意図しない別実験ログの誤ロードを防ぐため、ログ・設定ファイルは選択対象フォルダの直下に配置してください）。
...
| `weight` (重み行列) | 知識ノード間を繋ぐエッジ線（動的太さ・カラー強調 ＆ 数値ラベルバッジ表示） | ステップ表示画面（ネットワーク図） |
| `plan` (選択プラン) | 対象ノードの周囲に**金色二重リング**を描画、折れ線グラフ上の**選択知識マーカー（金色サークル＋活性値バッジ）・凡例ハイライト** | ステップ表示画面 / 折れ線グラフ画面 / `P` キー（知識選択ジャンプ） |
